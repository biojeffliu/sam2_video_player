...
class VideoSegmenter:
    ...
    def show_help_overlay(self, img):
        ...
        - Save segmentations (o)    : Saves object segmentations in provided output folder, as .json
        - Delete mask (x)           : Deletes current object's mask for the current frame
        """
    ...
    def segment_points(self, ann_frame_idx):
        ...
        for i, out_obj_id in enumerate(out_obj_ids):
            ...
            binary_mask = (mask_prob > 0.5).int()
            binary_mask_numpy = binary_mask.squeeze().cpu().numpy()
            if ann_frame_idx not in self.frame_masks:
                self.frame_masks[ann_frame_idx] = {}
            if out_obj_id not in self.frame_masks[ann_frame_idx]:
                self.frame_masks[ann_frame_idx][out_obj_id] = {}
            self.frame_masks[ann_frame_idx][out_obj_id] = binary_mask_numpy
            print('outmask_logits', out_mask_logits)
            print('mask_probabilities', mask_prob)
            print('binary mask: ', binary_mask)
            print('binary_mask_numpy: ', binary_mask_numpy)

        return

        # mask_probabilities = torch.sigmoid(out_mask_logits)
        # print('mask_probabilities', mask_probabilities)
        ...
        # if ann_frame_idx not in self.frame_masks:
        #     self.frame_masks[ann_frame_idx] = {}
        # if self.current_object_id not in self.frame_masks[ann_frame_idx]:
        #     self.frame_masks[ann_frame_idx][self.current_object_id] = {}
        # self.frame_masks[ann_frame_idx][self.current_object_id] = binary_mask_numpy
        # # print(self.frame_masks[self.current_frame], 'ikikjoiji')
    ...
    def propagate_masks(self):
        ...
        for out_frame_idx, out_obj_ids, out_mask_logits in self.predictor.propagate_in_video(self.inference_state):
            for i, out_obj_id in enumerate(out_obj_ids):
                ...
                binary_mask = (mask_prob > 0.5).int()
                binary_mask_numpy = binary_mask.squeeze().cpu().numpy()
                if out_frame_idx not in self.frame_masks:
                    self.frame_masks[out_frame_idx] = {}
                if out_obj_id not in self.frame_masks[out_frame_idx]:
                    self.frame_masks[out_frame_idx][out_obj_id] = {}
                self.frame_masks[out_frame_idx][out_obj_id] = binary_mask_numpy
    ...
    #             mask_prob = torch.sigmoid(out_mask_logits[i])
    #             binary_mask = (mask_prob > 0.5).int().squeeze().cpu().numpy()
    #             frame_masks_dict[out_obj_id] = binary_mask
    #         self.frame_masks[out_frame_idx] = frame_masks_dict
    #         video_segments[out_frame_idx] = frame_masks_dict
    #     print("Masks propagated for video.")
    #     self.predictor.reset_state(self.inference_state)
    #     return video_segments
    ...
    def video_player(self):
        ...
        while True:
            ...
            if viewing_frame in self.frame_masks:
                # Get the dictionary for the current frame: {obj_id: 2D mask}
                masks_dict = self.frame_masks[viewing_frame]
                # Create an overlay image (same shape as the frame)
                mask_vis = np.zeros_like(img)
                # Define a list of colors for different objects
                colors = [(180, 105, 255), (255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0)]
                # Iterate over each object and its corresponding mask
                for obj_id, mask in masks_dict.items():
                    # Ensure the mask is binary (0s and 1s)
                    binary_mask = (mask > 0).astype(np.uint8)
                    # Choose a color for the current object (cycling through the list)
                    color = np.array(colors[(obj_id - 1) % len(colors)], dtype=np.uint8)
                    # Create a colored overlay for the mask
                    colored_mask = np.stack([binary_mask]*3, axis=-1) * color
                    # Blend the colored mask onto the overlay
                    mask_vis = cv2.addWeighted(mask_vis, 1.0, colored_mask, 0.5, 0)
                # Finally, overlay the mask visualization on the original image
                img = cv2.addWeighted(img, 1, mask_vis, 0.5, 0)
            ...
            if key == ord('q'):
                ...
                print(f'Saving segmentations.')
                if not self.output_folder:
                    print("Output folder not provided. Please set output_folder.")
                else:
                    if not os.path.exists(self.output_folder):
                        os.makedirs(self.output_folder)
                    for frame_idx, seg_dict in self.frame_masks.items():
                        serializable_dict = {}
                        for obj_id, mask in seg_dict.items():
                            serializable_dict[str(obj_id)] = mask.tolist()
                        output_path = os.path.join(self.output_folder, f"{frame_idx}.json")
                        with open(output_path, "w") as f:
                            json.dump(serializable_dict, f)
                    print(f"Segmentations saved in folder: {self.output_folder}")
            elif key == ord('x'):  
                if self.current_frame in self.frame_masks and self.current_object_id in self.frame_masks[self.current_frame]:
                    del self.frame_masks[self.current_frame][self.current_object_id]
                    if not self.frame_masks[self.current_frame]:
                        del self.frame_masks[self.current_frame]
                    print(f"Deleted mask for object ID {self.current_object_id} at frame {self.current_frame}.")
                else:
                    print(f"No mask to delete for object ID {self.current_object_id} at frame {self.current_frame}.")
