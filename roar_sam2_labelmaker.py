        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if not self.output_folder:
                    ...
                    sample_shape = None
                    for frame_data in self.frame_masks.values():
                        for mask in frame_data.values():
                            sample_shape = mask.shape
                            break
                        if sample_shape:
                            break
                    if sample_shape is None:
                        print("No masks found. Nothing to save.")
                        return

                    max_obj_id = self.current_object_id  # Or manually set max known object ID

                    for frame_idx in range(len(os.listdir(self.input_folder))):
                        serializable_dict = {}
                        for obj_id in range(1, max_obj_id + 1):
                            if frame_idx in self.frame_masks and obj_id in self.frame_masks[frame_idx]:
                                mask = self.frame_masks[frame_idx][obj_id]
                            else:
                                mask = np.zeros(sample_shape, dtype=np.uint8)
...
if __name__ == "__main__":
    ...
    # input_folder = "./videos/april17_1557_22"
    # output_folder = "./output/april17_1557_22masks"
    input_format = ""
